...
import type { DetailField, ReportDetail } from "./reports.contract";
...
export const fetchStudentDetail = async (
  ...
  financial: { invoiced: number; paid: number; remaining: number };
} | null> => {
  ...
  const counts = countsFromGroupBy(
    attendanceRows.map((row) => ({ status: row.status, _count: row._count })),
  );

  const invoiced = toNumber(oldDebts._sum.total);
  const remaining = toNumber(oldDebts._sum.remaining);
  ...
  const academic: DetailField[] = [
    {
      label: "الطور",
      value:
        groupLevel?.educationStage.name ??
  ...
  const registration: DetailField[] = [
    {
      label: "رسوم التسجيل",
    ...
    },
    {
      ...
      value: student.registrationFeeAmount
        ? toNumber(student.registrationFeeAmount)
        : null,
      type: "money",
    },
    {
  ...
  const attendanceSummary = attendanceMetric(counts);
        ...
        ? [
            {
              ...
              title: "ملاحظة",
              fields: [
                { label: "الملاحظة", value: student.note, type: "text" as const },
              ],
            },
          ]
        : []),
    ...
    tables: [
      {
        ...
        columns: [
          ...
          column("enrolledAt", "تاريخ التسجيل", "date"),
          column("status", "الحالة", "status"),
          column("note", "ملاحظة", "text"),
        ],
      ...
      },
      {
        ...
        title: "الفواتير",
        columns: [
          column("invoiceNumber", "الرقم", "text"),
          column("subject", "المادة", "text"),
          column("month", "الشهر", "number"),
        ...
        rows: invoices.map((invoice) => ({
          ...
          subject:
            invoice.studentEnrollment.teachingAssignment.subject.name,
          month: invoice.month,
          year: invoice.year,
      ...
      },
      {
        ...
        title: "الدفعات",
        columns: [
          column("paymentNumber", "الرقم", "text"),
          column("amount", "المبلغ", "money"),
          column("paymentMethod", "الطريقة", "status"),
          column("paymentDate", "التاريخ", "date"),
...
export const fetchTeacherDetail = async (
  ...
  const [assignments, settlements, shares, payments] = await Promise.all([
    ...
    prisma.teacherDebtShare.findMany({
  ...
  const [settlementTotal, shareTotal, allocationTotal] = await Promise.all([
    ...
    prisma.teacherDebtShare.aggregate({
      where: { teacherId, status: { not: "CANCELLED" } },
      _sum: { shareAmount: true },
      _count: true,
    }),
    prisma.teacherPaymentAllocation.aggregate({
  ...
  const entitlement =
    toNumber(settlementTotal._sum.teacherAmount) +
    toNumber(shareTotal._sum.shareAmount);
  const paid = toNumber(allocationTotal._sum.amount);

  const studentCount = assignments.reduce(
    (sum, row) => sum + row._count.enrollments,
    0,
  );
  ...
  const detail: ReportDetail = {
    ...
    sections: [
      ...
      },
      {
        ...
        fields: [
          ...
          {
            label: "من التخليص",
            value: toNumber(settlementTotal._sum.teacherAmount),
            type: "money",
          },
          {
            label: "من حصص الدَّين",
            value: toNumber(shareTotal._sum.shareAmount),
            type: "money",
          },
    ...
    tables: [
      ...
      },
      {
        ...
        title: "التخليصات",
        columns: [
          column("settlementNumber", "الرقم", "text"),
          column("subject", "المادة", "text"),
          column("sheet", "الكشف", "text"),
          column("teacherAmount", "المستحقّ", "money"),
          column("allocated", "المدفوع", "money"),
          column("status", "الحالة", "status"),
          column("computedAt", "حُسب في", "date"),
        ],
        rows: settlements.map((row) => ({
          ...
          teacherAmount: toNumber(row.teacherAmount),
          allocated: row.teacherAllocations.reduce(
      ...
      },
      {
        ...
        columns: [
          column("period", "فترة الأصل", "text"),
          column("collectedAt", "تاريخ التحصيل", "date"),
          column("basis", "الأساس", "status"),
          column("shareAmount", "الحصّة", "money"),
          column("status", "الحالة", "status"),
        ],
        rows: shares.map((row) => ({
          id: row.id,
          period: `${row.debtCollection.originalYear}-${String(
            row.debtCollection.originalMonth,
          ).padStart(2, "0")}`,
...
export const fetchSettlementDetail = async (
  settlementId: string,
): Promise<{ detail: ReportDetail; teacherAmount: number } | null> => {
    ...
    sections: [
      {
        ...
        fields: [
          ...
          },
          {
            label: "الكشف",
            value:
              settlement.attendanceSheet.label ??
        ...
        fields: [
          ...
          },
          {
            ...
            value: settlement.percentageSnapshot
              ? toNumber(settlement.percentageSnapshot)
              : null,
            type: "percent",
          },
          {
            ...
            value: settlement.perStudentSnapshot
              ? toNumber(settlement.perStudentSnapshot)
              : null,
            type: "money",
          },
          {
            ...
            value: settlement.perSessionSnapshot
              ? toNumber(settlement.perSessionSnapshot)
              : null,
            type: "money",
          },
        ],
      },
      {
        ...
        fields: [
          {
            label: "الرسم الشهري",
            value: toNumber(settlement.tuitionSnapshot),
            type: "money",
          },
          {
          ...
          {
            label: "إجمالي الرسوم",
            value: toNumber(settlement.grossTuitionSnapshot),
            type: "money",
          },
          {
            label: "المحصَّل",
            value: toNumber(settlement.collectedSnapshot),
            type: "money",
          },
          {
            label: "المتبقّي",
            value: toNumber(settlement.remainingSnapshot),
            type: "money",
          },
        ],
      },
    ...
    tables: [
      {
        ...
        columns: [
          ...
          column("sessionDate", "التاريخ", "date"),
          column("countedStudents", "الطلبة المحتسبون", "number"),
          column("rate", "نصيب الوحدة", "money"),
          column("lineTotal", "مجموع البند", "money"),
        ],
