...
export const getTeacherStatementService = async (
  teacherId: string,
  academicYearId: string,
) => {
  const teacher = await prisma.teacher.findUnique({
    where: { id: teacherId },
    select: Prisma.validator<Prisma.TeacherSelect>()({
      id: true,
      firstName: true,
      lastName: true,
      phone: true,
  ...
  const rows = sheets.map((sheet) => {
     ...
     */
    const settlement =
      sheet.settlements.find(
        (s) => s.teachingAssignmentId === sheet.teachingAssignmentId,
      ) ?? null;

    const allocation = settlement?.teacherAllocations.find(
      (a) => a.teacherPayment.status !== "CANCELLED",
    );
   ...
   */
  const shares = await prisma.teacherDebtShare.findMany({
    where: {
      teacherId,
