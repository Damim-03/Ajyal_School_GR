...
const shareSelect = Prisma.validator<Prisma.TeacherDebtShareSelect>()({
  id: true,
  teacherId: true,
  basisSnapshot: true,
...
const toResponse = <T extends Record<string, unknown>>(share: T) => ({
  ...share,
  percentageSnapshot: Number(share.percentageSnapshot),
  unitRateSnapshot:
    share.unitRateSnapshot === null ? null : Number(share.unitRateSnapshot),
  collectedAmount: Number(share.collectedAmount),
  shareAmount: Number(share.shareAmount),
});

// --------------------------------------------------
// التسجيل — يقع داخل معاملة دفعة الطالب
// --------------------------------------------------
 ...
 */
export const recordDebtCollections = async (
  tx: Tx,
  paymentId: string,
  collections: { invoiceId: string; paidAmount: Prisma.Decimal }[],
  collectedAt: Date,
) => {
  for (const entry of collections) {
    ...
    let attendedUnits: number | null = null;
    let shareAmount: Prisma.Decimal;

    if (policy.debtShareBasis === "ATTENDED_UNITS") {
      /* سعرُ الحصة للمؤسسة وقتها × نسبة الأستاذ = نصيبُ حضورٍ واحد */
      const sessionRate =
        settlement.approvedSessionsSnapshot > 0
    ...
    if (shareAmount.lessThanOrEqualTo(0)) continue;
...
export const listDebtSharesService = async (query: DebtShareQueryInput) => {
  const { skip, take, page, limit } = getPagination(query.page, query.limit);

  const where: Prisma.TeacherDebtShareWhereInput = {
  ...
  const [shares, total] = await Promise.all([
    ...
    prisma.teacherDebtShare.count({ where }),
  ]);
...
export const cancelDebtShareService = async (
  id: string,
  body: CancelDebtShareInput,
  userId: string,
) => {
  const share = await prisma.teacherDebtShare.findUnique({
    where: { id },
  ...
  if (share.status === "PAID") {
    throw new ConflictException(
      "A paid debt share cannot be cancelled — cancel the teacher payment instead",
      ErrorCodeEnum.VALIDATION_ERROR,
    );
  }

  const updated = await prisma.teacherDebtShare.update({
    where: { id },
    data: {
      status: "CANCELLED",
