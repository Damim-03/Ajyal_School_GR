 ...
 * حساب مستحقّ الأستاذ.
 *
 * دالّة خالصة عمداً: لا قاعدة بيانات ولا وقت حاضر. كل ما تحتاجه يصل
 * إليها مُجمَّعاً، فتُختبر بالأرقام وحدها — وهو ما يجعل مراجعة مبلغٍ
 * مشكوكٍ فيه ممكنةً دون تشغيل النظام كلّه.
 *
 * ولا رقم هنا: النسبة والمبلغ وعدد الحصص كلها تصل من السياسة.
 */

export interface SettlementInputs {
  method: SettlementMethod;
  countBasis: SettlementCountBasis;
  ...
  /** عدد الحصص المعتمد للكشف — القاسم عند تفتيت الحقّ الشهري */
  approvedSessions: number;

  enrolledCount: number;
  paidCount: number;

  /** الحصص المحتسبة، مرتّبة */
  sessions: {
    sessionId: string | null;
    lessonNumber: number;
    sessionDate: Date | null;
    /** كلُّ من حضر — للعرض والتقارير */
    presentCount: number;
    /**
     * من حضر **ودخل الحساب** وفق `countBasis`.
     *
     * يساوي `presentCount` مع `ENROLLED` و`PRESENT`، ويَنقص عنه مع
     * `PAID` بمقدار حضور المخلَّفين.
     */
    countedCount: number;
  }[];
}
...
export interface SettlementResult {
  ...
  /** مجموع (طالب × حصة حضرها) — الحضور الخام بلا تصفية */
  attendedUnits: number;
  /** ما دخل الحساب منه فعلاً وفق `countBasis` — وهو الذي يفسّر المبلغ */
  countedUnits: number;
  /** عدد الطلبة الذي دخل الحساب فعلاً وفق countBasis */
  countedStudents: number;
}
...
const resolveCountedStudents = (input: SettlementInputs): number => {
  switch (input.countBasis) {
    ...
    case "PRESENT":
      ...
      return input.sessions.length > 0
        ? Math.round(
            input.sessions.reduce((sum, s) => sum + s.presentCount, 0) /
...
export const computeSettlement = (
  ...
  const attendedUnits = input.sessions.reduce(
    (sum, s) => sum + s.presentCount,
    0,
  );

  const countedUnits = input.sessions.reduce(
    (sum, s) => sum + s.countedCount,
    0,
  );
  ...
  switch (input.method) {
    ...
    case "PERCENTAGE": {
      const gross =
        input.countBasis === "PAID"
          ? input.tuition.times(input.paidCount)
          : input.tuition.times(countedStudents);

      const teacherAmount = roundMoney(gross.times(pct), mode, dp);

      const shares = distributeEvenly(teacherAmount, sessionCount, mode, dp);
    ...
    case "PER_STUDENT": {
      ...
      const teacherAmount = roundMoney(
        perStudent.times(countedStudents),
        mode,
        dp,
      );
    ...
    // ------------------------------------------------
    case "PER_SESSION": {
      const perSession = roundMoney(
        input.amountPerSession ?? ZERO,
        mode,
        dp,
      );
    ...
    case "PER_ATTENDED_SHARE": {
      const perSessionShare =
        input.approvedSessions > 0
      ...
      const teacherAmount = roundMoney(rate.times(countedUnits), mode, dp);
       ...
       */
      const raw = input.sessions.map((session) =>
        roundMoney(rate.times(session.countedCount), mode, dp),
      );

      const drift = teacherAmount.minus(
        raw.reduce((sum, value) => sum.plus(value), ZERO),
      );
      ...
      const lines = input.sessions.map((session, i) => ({
        ...
        lineTotal:
          i === raw.length - 1
            ? roundMoney(raw[i]!.plus(drift), mode, dp)
            : raw[i]!,
      }));
 ...
 */
export const requiredFieldFor = (
  method: SettlementMethod,
): keyof Pick<
  SettlementInputs,
  "teacherPercentage" | "amountPerStudent" | "amountPerSession"
> => {
  switch (method) {
    case "PER_STUDENT":
