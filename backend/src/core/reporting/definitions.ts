import { EXTRA_METRIC_DEFINITIONS } from "./definitions.extra";

// ======================================================
// كتالوج المؤشّرات — §66 و§70
//
// التوثيقُ هنا بياناتٌ لا تعليقات، لسببين:
//
//   1. §70 يطلب عرضَ تعريف المؤشّر عند المرور عليه. والتعريفُ
//      المكتوبُ في تعليقٍ لا يصل إلى الواجهة، فيُعاد كتابتُه هناك
//      ثم يتباعد النصّان عند أوّل تعديل.
//
//   2. تعريفٌ يُقرأ في المراجعة أسهل من تعريفٍ يُستخرج من قراءة
//      دالّة. ومَن يراجع صحّةَ «نسبة التحصيل» يقرأ سطراً واحداً.
//
// و`formula` مكتوبةٌ بمسمّيات المخطّط لا بالرياضيات المجرّدة، ليُمكن
// تتبّعُها إلى العمود الذي تقرؤه فعلاً.
// ======================================================
...
export type MetricUnit = "count" | "money" | "percent" | "ratio";

export type MetricDirection =
  /** الارتفاع أفضل — الإيراد، الحضور */
  | "higherIsBetter"
  /** الانخفاض أفضل — الدَّين، الغياب */
  | "lowerIsBetter"
  /** لا حكم — عددُ الطلبة ليس جيّداً ولا سيّئاً بذاته */
  | "neutral";
...
export type MetricDefinition = {
  ...
  /** الصيغة بمسمّيات المخطّط */
  formula: string;
  /** شرحٌ للإدارة لا للمبرمج */
  description: string;
  /** ما يُستثنى ولماذا — §52 */
  exclusions?: string;
  /** ما لا يعنيه المؤشّر — يمنع سوءَ القراءة */
  caveat?: string;
  /** المسار الذي يفتحه النقر — §40 */
  drillTo?: string;
};
...
export const METRIC_DEFINITIONS: MetricDefinition[] = [
  ...
  },
  {
    ...
    caveat:
      "يُنسب إلى شهر الفاتورة لا يوم الدفع: دفعةُ نوفمبر لفاتورة سبتمبر تظهر في سبتمبر. للنقد الداخل فعلاً انظر «حركة النقد».",
  ...
  },
  {
    ...
    direction: "lowerIsBetter",
    formula:
      "SUM(Invoice.remaining) WHERE (Invoice.year, Invoice.month) < الفترة المرجعية",
    description: "متبقّي فواتير فتراتٍ سابقة للفترة المعروضة.",
    caveat:
      "المقارنة بشهر الفاتورة وسنتها لا بتاريخ إنشائها — فاتورة سبتمبر المُدخَلة في أكتوبر دَينُها سبتمبريّ (§58).",
  },
  {
    ...
    direction: "higherIsBetter",
    formula:
      "SUM(DebtCollection.collectedAmount) WHERE Payment.status = ACTIVE",
  ...
  // --------------------------------------------------
  {
    ...
    caveat:
      "المتأخّر يُحتسب حاضراً لأنه تلقّى الدرس؛ انضباطُه في «نسبة التأخّر». والمعذور يبقى في المقام لئلّا يظهر فوجٌ نصفُه بأعذار بحضورٍ كامل.",
  ...
  // --------------------------------------------------
  {
    ...
    direction: "neutral",
    formula:
      "SUM(Settlement.teacherAmount) + SUM(TeacherDebtShare.shareAmount) WHERE status != CANCELLED",
    ...
    caveat:
      "يشمل المسوّدات. للالتزام الفعلي على المؤسسة انظر «المستحقّ المعتمَد» — المسوّدة حسابٌ لم يُعتمد.",
  ...
  },
  {
    ...
    direction: "neutral",
    formula:
      "SUM(TeacherPaymentAllocation.amount) WHERE TeacherPayment.status = ACTIVE",
    description: "ما دُفع فعلاً، محسوباً من التخصيصات لا من مجاميع الدفعات.",
    exclusions: "دفعات الأساتذة الملغاة لا تدخل (§52.3).",
    caveat:
      "منفصلٌ تماماً عن دفعات الطلبة — هذا صادرٌ من المؤسسة وذاك واردٌ إليها (§52.5).",
  ...
  // --------------------------------------------------
  {
    ...
    caveat:
      "ليس ربحاً. النظام لا يعرف الإيجار ولا الكهرباء ولا الأجور الإدارية، فتسميتُه ربحاً تُخفي مصاريف حقيقية (§33).",
...
export const METRICS_BY_KEY: ReadonlyMap<string, MetricDefinition> = new Map(
  ALL_METRIC_DEFINITIONS.map((metric) => [metric.key, metric]),
);

export const metricsByDomain = (domain: MetricDomain): MetricDefinition[] =>
  ALL_METRIC_DEFINITIONS.filter((metric) => metric.domain === domain);
