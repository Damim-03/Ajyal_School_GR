...
exports.explainMissingPolicy = exports.resolveSettlementPolicy = exports.describePolicyScope = exports.policySpecificity = exports.buildPolicyScopeKey = exports.SCOPE_NONE = void 0;
...
const W_GROUP = 2;
const W_SUBJECT = 1;
const buildPolicyScopeKey = (scope) => [
    `yr:${scope.academicYearId}`,
    `sub:${scope.subjectId ?? exports.SCOPE_NONE}`,
    `grp:${scope.studyGroupId ?? exports.SCOPE_NONE}`,
    `tch:${scope.teacherId ?? exports.SCOPE_NONE}`,
].join("|");
exports.buildPolicyScopeKey = buildPolicyScopeKey;
const policySpecificity = (scope) => (scope.teacherId ? W_TEACHER : 0) +
    (scope.studyGroupId ? W_GROUP : 0) +
    (scope.subjectId ? W_SUBJECT : 0);
exports.policySpecificity = policySpecificity;
const describePolicyScope = (scope) => {
    const parts = [];
    if (scope.teacher)
...
exports.describePolicyScope = describePolicyScope;
/**
 ...
 */
const resolveSettlementPolicy = async (target, onDate, client = client_1.prisma) => {
    ...
    return candidates
        .map((policy) => ({
        ...policy,
        specificity: (0, exports.policySpecificity)(policy),
    }))
        .sort((a, b) => b.specificity - a.specificity ||
        b.effectiveFrom.getTime() - a.effectiveFrom.getTime())[0];
};
exports.resolveSettlementPolicy = resolveSettlementPolicy;
 ...
 */
const explainMissingPolicy = async (target, onDate, client = client_1.prisma) => {
    ...
    if (all.length === 0) {
        return ("لا سياسةَ تخليصٍ واحدة في هذه السنة الدراسية — " +
    ...
    const lines = all.map((policy) => {
        ...
        if (policy.studyGroupId && policy.studyGroupId !== target.studyGroupId)
            why.push(`نطاقُها فوج ${policy.studyGroup.level.name} · ${policy.studyGroup.name}`);
        if (policy.teacherId && policy.teacherId !== target.teacherId)
            why.push(`نطاقُها الأستاذ ${policy.teacher.lastName} ${policy.teacher.firstName}`);
        return `• «${policy.name}» — ${why.length > 0 ? why.join("، ") : "نطاقُها لا يشمل هذا الإسناد"}`;
    });
    return (`توجد ${all.length} سياسة في هذه السنة ولا واحدةَ منها تنطبق على هذا الكشف:\n` +
        lines.join("\n") +
        `\nصحّح نطاقَ إحداها أو أضِف سياسةً تشمله.`);
};
exports.explainMissingPolicy = explainMissingPolicy;
//# sourceMappingURL=settlement-scope.js.map