...
exports.serialize = exports.CONTENT_TYPE = exports.exportFilename = exports.toXlsx = exports.toCsv = exports.selectColumns = void 0;
 ...
 */
const selectColumns = (columns, requested) => {
...
const toCsv = (report, options = {}) => {
    const lines = [];
    const push = (cells) => lines.push(cells.map(csvField).join(CSV_DELIMITER));
    /*
     * ترويسةُ السياق — §42.
     *
    ...
    if (report.table) {
        const columns = (0, exports.selectColumns)(report.table.columns, options.columns);
        push(columns.map((column) => column.label));
        for (const row of report.table.rows) {
            push(columns.map((column) => cellValue(row[column.key], column.type)));
        }
    }
    /*
     * BOM في أوّل الملفّ.
...
const cellXml = (value, reference, bold) => {
    if (value === null || value === "") {
     ...
     */
    return `<c r="${reference}" t="inlineStr"${bold ? ' s="1"' : ""}><is><t xml:space="preserve">${escapeXml(String(value))}</t></is></c>`;
};
const sheetXml = (rows) => {
    const body = rows
        .map((row, rowIndex) => {
        const cells = row.cells
            .map((cell, columnIndex) => cellXml(cell, `${columnLetter(columnIndex)}${rowIndex + 1}`, row.bold ?? false))
            .join("");
...
const toXlsx = (report, options = {}) => {
    ...
    if (report.table) {
        ...
        for (const row of report.table.rows) {
            rows.push({
                cells: columns.map((column) => cellValue(row[column.key], column.type)),
            });
        }
    }
...
<workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets><sheet name="التقرير" sheetId="1" r:id="rId1"/></sheets></workbook>`, "utf8"));
...
<styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><fonts count="2"><font><sz val="11"/><name val="Calibri"/></font><font><b/><sz val="11"/><name val="Calibri"/></font></fonts><fills count="1"><fill><patternFill patternType="none"/></fill></fills><borders count="1"><border/></borders><cellStyleXfs count="1"><xf numFmtId="0" fontId="0" fillId="0" borderId="0"/></cellStyleXfs><cellXfs count="2"><xf numFmtId="0" fontId="0" fillId="0" borderId="0" xfId="0"/><xf numFmtId="0" fontId="1" fillId="0" borderId="0" xfId="0" applyFont="1"/></cellXfs></styleSheet>`, "utf8"));
...
const serialize = (report, format, options = {}) => (format === "csv" ? (0, exports.toCsv)(report, options) : (0, exports.toXlsx)(report, options));
exports.serialize = serialize;
//# sourceMappingURL=reports.export.js.map